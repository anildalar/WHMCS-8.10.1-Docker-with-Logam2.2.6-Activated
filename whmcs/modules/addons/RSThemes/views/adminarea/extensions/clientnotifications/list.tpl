...
{block name="template-content"}
    <div class="t-c__top top m-b-3x" data-top-search="" data-toggler-options="toggleClass: is-open;">
        <div class="top__toolbar">
            <h3 class="section__title">
                Instances
                {include file="adminarea/includes/helpers/docs.tpl" link=$extension->getDocs()->instances['instances']}
            </h3>
        </div>
        <div class="top__toolbar is-right">
            ...
            <button data-notification-import-btn class="btn btn--secondary {if !class_exists("ZipArchive")} is-disabled{/if}">
    ...
    <div class="section">
        <div class="t-c__body t-c__body--boxed">
            <form class="dataTables_wrapper no-footer is-hidden" action="{$helper->url('Template@extension',['templateName' => $template->getMainName(), 'extension' => $extension->getLinkName()])}" method="POST">
                <table class="t-c__table table mob-table--block dataTable no-footer" id="styles-table" data-services-table data-search-input="#table-search" data-clickable-rows="true" data-responsive="false">
                    ...
                    <tbody>
                        {if $extension->getClientNotifications()|@count == 0}
                            {* Brak *}
                        {else}
                            {foreach from=$extension->getClientNotifications() item=style key=k}
                                {assign var="alertData" value=json_decode($style->date)}
                                <tr>
                                    <td class="cell-name">
                                        <b> <a href="{$helper->url('Template@extension',['templateName' => $template->getMainName(), 'extension' => $extension->getLinkName(), 'id' => $style->id, 'exaction' => "item"])}">{$style->name}</a></b>
                                    </td>
                                    <td class="cell-scheme">
                                        {if $alertData->start}{$style->getFormattedStartDate()}{else}<span class="cell-empty-date">-</span>{/if}
                                    </td>
                                    <td class="cell-scheme">
                                        {if $alertData->end}{$style->getFormattedEndDate()}{else}<span class="cell-empty-date">-</span>{/if}
                                    </td>
                                    <td class="cell-scheme">
                                        {if $style->enabled}
                                    ...
                                    <td class="cell-actions">
                                        <div class="has-dropdown">
                                            <a class="btn btn--default btn--outline btn--xs" href="" data-toggle="lu-dropdown" data-placement="bottom right" data-notification-table-actions data-notification-id="{$style->id}">
                                            ...
                                            <div class="dropdown" data-dropdown-menu>
                                                ...
                                                <div class="dropdown__menu">
                                                    <ul class="nav">
                                                        <li class="nav__item">
                                                            <a class="nav__link" href="{$helper->url('Template@extension',['templateName' => $template->getMainName(), 'extension' => $extension->getLinkName(), 'id' => $style->id, 'exaction' => "item"])}">
                                                                <span class="nav__link-icon ls ls-pen"></span>
                                                                <span class="nav__link-text">Manage</span>
                                                            </a>
                                                        </li>
                                                        <li class="nav__item">
                                                            <a class="nav__link" href="{$helper->url('Template@extension',['templateName' => $template->getMainName(), 'extension' => $extension->getLinkName(), 'id' => $style->id, 'exaction' => "status"])}">
                                                                {if !$style->enabled}
                                                        ...
                                                        <li class="nav__item">
                                                            <a class="nav__link" href="{$helper->url('Template@extension',['templateName' => $template->getMainName(), 'extension' => $extension->getLinkName(), 'exaction' => 'duplicate', 'id'=> $style->id])}">
                                                        ...
                                                        <li class="nav__item">
                                                            <a class="nav__link" href="#modal-delete-client-notification" data-notification-remove-{$style->id} data-href="{$helper->url('Template@extension',['templateName' => $template->getMainName(), 'extension' => $extension->getLinkName(), 'id' => $style->id, 'exaction' => "delete"])}" data-toggle="lu-modal">
    ...
    <form data-notification-import-form enctype="multipart/form-data" action="{$helper->url('Template@extension',['templateName' => $template->getMainName(), 'extension' => $extension->getLinkName(), 'exaction' => "import"])}" method="POST">
