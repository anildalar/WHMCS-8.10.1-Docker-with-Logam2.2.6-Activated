    ...
    <h5> Promotion type: <strong>Banner</strong></h5>
        <form id="saveConfig" action="{$helper->url('Template@extension',['templateName'=>$template->getMainName(),'extension'=> $extension->getLinkName()])}" name="saveGlobalConfig" method="post">
            ...
            <div class="panel panel--collapse">
                <div class="collapse-toggle">
                    <h6 class="top__title">
                        Disable automatic slide of banner slides
                        {include file="adminarea/includes/helpers/tooltip.tpl" tooltip='Will disable automatic changing of slides on banner'}
                    </h6>
                    <label>
                        <div class="switch">
                            <input type="hidden" name="autoslide_disable" value="0">
                            <input class="switch__checkbox" type="checkbox" name="autoslide_disable" {if $extension->getConfig('autoslide_disable') == '1'}checked{/if} value="1">
                            <span class="switch__container"><span class="switch__handle"></span></span>
                        </div>
                    </label>
                </div>
