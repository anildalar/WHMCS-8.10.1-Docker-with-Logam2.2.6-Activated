...
class License
{
    ...
    private $licenseKeyName;
    /** @var string $licenseSecretKey */
    private $licenseSecretKey;
    /** @var string $licenseConfigKey */
    private $licenseConfigKey;
    ...
    private $validateDetails = ["status", "service_status", "license_status", "nextduedate", "version", "lastRemoteChecked", "regdate", "first_payment_amount", "recuring_amount", "payment_method"];
    /** @var array $rawLicenseDetails */
    private $rawLicenseDetails;
    /** @var string $licenseEncoded */
    private $licenseEncoded;
    /** @var string $licenseWarningKey */
    private $licenseWarningKey;
    /** @var string $licenseWarningMessage */
    private $licenseWarningMessage;
    /** @var bool $debug */
    private $debug = false;
    /**
     * @var string
     */
    private $checkLicenseHour;
    ...
    public static $licenseServerUrl;
    ...
    private function prepareLicense()
    {
        ...
        $this->licenseDetails = [
            ...
            "version" => "2.3.0-b1",
            "fullversion" => "2.3.0-b1",
            "lastRemoteChecked" => '2024-11-08',
            "lastRemoteCheckedFail" => "",
            "lastRemoteCheckedSuccess" => "",
            "regdate" => date("Y-m-d"),
            "first_payment_amount" => "0.00",
            "recuring_amount" => "0.00",
            "payment_method" => "Nulled by WonrayWildSparks on Babiato.co",
            "warningShowDate" => "",
            "deactivationDate" => "",
            "license" => "NULLED-BY-WONRAYWILDSPARKS-ON-BABIATO"
        ];
        ...
        if($this->licenseDetails["service_status"] == "Active" && $this->remoteCheck()) {
            $this->reloadRemote();
        } elseif(!isset($this->licenseDetails["license"]) || empty($this->licenseDetails["license"])) {
            $this->reloadRemote();
        }
        $licenseValidated = true;
        $data = $this->decode($this->licenseDetails["license"]);
        preg_match_all("/<(.*?)>([^<]+)<\\/\\1>/i", $data, $matches);
        foreach ($matches[1] as $k => $v) {
            $licenseResults[$v] = $matches[2][$k];
        }
        $licenseResults["lastRemoteChecked"] = $this->decode($licenseResults["encryptedCurrentClientDate"]);
        foreach ($this->validateDetails as $item) {
            if(!isset($licenseResults[$item]) || !isset($this->licenseDetails[$item])) {
                $licenseValidated = false;
            } elseif($licenseResults[$item] != $this->licenseDetails[$item]) {
                $licenseValidated = false;
            }
        }
        if(!$licenseValidated) {
    ...
    {
        /*$details = $this->loadRemoteLicense($this->licenseKey, $this->template->getVersion(), $this->templateName);
    ...
    private function decode($data)
    {
        ...
        dd($info);
        $chunks = str_split($data, $info["bits"] / 8);
        $out = "";
    ...
    private function loadRemoteLicense($licenseKey, $version, $templateName)
    {
        ...
        $verifyFilePath = "modules/servers/RSLicensing/verify_rsa.php";
        $licenseFields = ["licensekey" => $licenseKey, "domain" => self::getDomain(), "ip" => "", "dir" => self::getDirPath(), "version" => $version, "cron" => \RSThemes\Helpers\AddonHelper::isCliMode(), "template" => $templateName, "clientdate" => date("Y-m-d")];
        $query = "";
    ...
    private function syncExtensions($details)
    {
        ...
        foreach ($this->template->getExtensions() as $extension) {
            ...
            } elseif(method_exists($extension, "checkLicense") && !in_array($extension->name, $allowedExtensions) && $extension->isActive()) {
