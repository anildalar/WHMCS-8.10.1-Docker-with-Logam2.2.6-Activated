...
angular.module("mgCRMapp").controller(
        ...
        ['$rootScope', '$scope', '$translate', '$http', 'blockUI', 'ngDialog',
function( $rootScope,   $scope,   $translate,   $http,   blockUI,   ngDialog)
{
    ...
    if($rootScope.notifyCreated)
    {
        $scope.scopeMessages.push({
                type:   'success',
                title:   "Success!",
                content: 'The notification has been created successfully'
            });
    }
    ...
    $scope.callServer = function callServer(tableState) 
    {
        ...
        $http.post($rootScope.settings.config.apiURL + '/notifications/list/json', params).then(function(result) 
        {
    ...
    $scope.triggerDelete = function(id)
    {
        ...
        $scope.confirmDeleteDialog = ngDialog.openConfirm({
            template:'\
                <h2>{{ "delete.notification.message" | translate }}</h2>\
            ...
            // come on give me data from backend
            $http.post($rootScope.settings.config.apiURL + '/notifications/delete/'+ id +'/json', {
                cache: false,
                isArray: true
            }).then(function(result) 
            {
                ...
                $scope.scopeMessages.push({
                    type:    'success',
                    title:   "Success!",
                    content:  'The notification has been deleted successfully',
                });
