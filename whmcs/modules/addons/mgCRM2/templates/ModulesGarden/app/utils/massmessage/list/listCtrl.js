...
angular.module("mgCRMapp").controller(
        'massmessageListController',
        ['$rootScope', '$scope', '$translate', '$http', 'blockUI', 'isAllowed', 'ngDialog',
function( $rootScope,   $scope,   $translate,   $http,   blockUI,   isAllowed,   ngDialog)
{
    
    $scope.massmailTableBlock = blockUI.instances.get('massmailTable');
    ...
    if($rootScope.massMessageCreated)
    {
        $scope.scopeMessages.push({
                type:   'success',
                title:   "Success!",
                content: 'The mass message has been created successfully'
            });
    }
    
    if($rootScope.massMessageUpdated)
    {
        $scope.scopeMessages.push({
                type:   'success',
                title:   "Success!",
                content: 'The mass message has been updated successfully'
            });
    }
    ...
    $scope.callServer = function callServer(tableState) 
    {
        ...
        $scope.massmailTableBlock.start();
        ...
        $http.post($rootScope.settings.config.apiURL + '/massmessages/list/json', params).then(function(result) 
        {
            // update controller container for data from response
            $scope.displayed = result.data.data;
            for(var $i = 0; $i < $scope.displayed.length; $i++)
            {
                $scope.displayed[$i].date = moment($scope.displayed[$i].date).format('DD-MM-YYYY, HH:mm:ss');
            }
            // stop blockui indicator    
            $scope.massmailTableBlock.stop();
    ...
    $scope.openContentDialog = function(title, content)
    {
        ngDialog.open({
            template: '<h1><b>'+title+'</b></h1>' + content 
    ...
    $scope.triggerDelete = function(id)
    {
        ...
        $scope.confirmDeleteDialog = ngDialog.openConfirm({
            template:'\
                <h2>{{ "delete.massMessageConfiguration.message" | translate }}</h2>\
                <div class="ngdialog-buttons text-center">\
            ...
            // come on give me data from backend
            $http.post($rootScope.settings.config.apiURL + '/massmessages/delete/'+ id +'/json', {
                cache: false,
                isArray: true
            }).then(function(result) 
            {
                ...
                $scope.scopeMessages.push({
                    type:    'success',
                    title:   "Success!",
                    content:  'The mass message configuration has been deleted successfully',
                });
