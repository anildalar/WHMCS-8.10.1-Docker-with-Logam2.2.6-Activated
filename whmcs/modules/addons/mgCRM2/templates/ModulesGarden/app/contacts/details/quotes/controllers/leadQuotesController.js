...
angular.module("mgCRMapp").controller(
        'leadQuotesController',
        ['$scope', '$rootScope', '$state', '$stateParams', 'blockUI', '$http',
function( $scope,   $rootScope,   $state,   $stateParams,   blockUI,   $http)
{
    $scope.resource_id = $stateParams.id;
    // messages 
    $scope.scopeMessages = [];
    
    $scope.quotesTable = blockUI.instances.get('quotesTable');
    ...
    $scope.callServer = function callServer(tableState) 
    {
        ...
        $http.post($rootScope.settings.config.apiURL + '/lead/' + $stateParams.id + '/quotes/json', params).then(function(result) 
        {
            // update controller container for data from response
            $scope.displayed = result.data.data;
            // stop blockui indicator    
            $scope.quotesTable.stop();
    ...
    $scope.syncQuotes = function()
    {
        ...
        // come on give me data from backend
        $http.get($rootScope.settings.config.apiURL + '/lead/' + $stateParams.id + '/quotes/sync/json').then(function(result) 
        {
            angular.element('#quotes-main-search').trigger('input');
