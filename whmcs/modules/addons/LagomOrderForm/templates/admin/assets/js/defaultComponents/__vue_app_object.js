            ...
            methods: {
                ...
                vloadData : function (params, forceRawData) {
                    ...
                    return $.get(self.targetUrl, function(data){
                        if (!forceRawData) {
                            data = data.data;
                            if (data && data.callBackFunction && typeof window[data.callBackFunction] === "function") {
                ...
                loadModal : function(event, targetId, namespace, index, params, addSpinner, spinnerOnTargetId){
                    ...
                    if (addSpinner) {
                        self.showSpinner(event,targetId,spinnerOnTargetId);
                    }
                    self.refreshUrl();
                    ...
                    var request = $.get(self.targetUrl, function(data){
                        ...
                        if (addSpinner) {
                            self.hideSpinner(event,targetId,spinnerOnTargetId);
                        }
                ...
                showSpinner : function(event,targetId, spinnerOnTarget) {
                    var self = this;
                    var spinnerTarget = self.getSpinerTarget(event);
                    if(spinnerOnTarget && $('#'+targetId)){
                        var elWidth = $($('#'+targetId)).width();
                        var spinnerClass = $($('#'+targetId)).hasClass('lu-btn--success') ? 'lu-preloader-success' : ($('#'+targetId).hasClass('lu-btn--danger') ? 'lu-preloader-danger' : '');
                        $($('#'+targetId)).attr('originall-button-content', $($('#'+targetId)).html());
                        $($('#'+targetId)).html('<span class="lu-btn__icon lu-temp-button-loader" style="margin: 0 0 0 0 !important; width: ' + elWidth + 'px;"><i style="margin: 0 0 0 0 !important" class="lu-btn__icon lu-preloader lu-preloader--sm ' + spinnerClass + '"></i></span>');
                ...
                reloadModalContent: function(event, targetId, namespace, index, params, addSpinner) {
                    ...
                    axios.post(self.targetUrl, formData).then(function (data) {
                        data = data.data.data;
                        if(data.status === 'success'){
                           self.htmlContent = data.rawData.htmlData;
                ...
                hideSpinner : function(event,targetId,spinnerOnTargetId) {
                    var self = this;
                    var spinnerTarget = self.getSpinerTarget(event);
                    if(spinnerOnTargetId && $('#'+targetId)) {
                        $('#'+targetId).html($('#'+targetId).attr('originall-button-content'));
                        $('#'+targetId).removeAttr('originall-button-content');
                    }else if ($(event.target).attr('originall-button-content')) {
                ...
                addSpinner : function(event) {
                    ...
                    $(event.target).html('<span class="lu-btn__icon lu-temp-button-loader" style="margin: 0 0 0 0 !important; width: ' + elWidth + 'px;"><i style="margin: 0 0 0 0 !important" class="lu-btn__icon lu-preloader lu-preloader--sm ' + spinnerClass + '"></i></span>');
                },
