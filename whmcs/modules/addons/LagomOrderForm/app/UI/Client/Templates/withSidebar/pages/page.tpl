...
<div class="main-body preloaded">
    ...
    {/if}
    <div class="container">
        {if $rawObject->getElementById('promoCodeInfoComponent')}
            {$rawObject->insertElementById('promoCodeInfoComponent')}
        {/if}
        <div class="main-grid">
            <div id="baseOrderContent" class="main-content main-content-m-w">
                ...
                {foreach from=$rawObject->getIntegrationComponents() key=name item=id}
                    {if $rawObject->getElementById($id)}
                        {$rawObject->insertElementById($id)}
                    {/if}
                {/foreach}
            ...
            {if $rawObject->getElementById('cartComponent')}
                {$rawObject->insertElementById('cartComponent')}
            {/if}
        </div>
    </div>   
</div>