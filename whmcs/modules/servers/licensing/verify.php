...
if ($addonId) {
    ...
    $allowdomainconflict = (int) $model->productAddon->moduleConfiguration()->where("setting_name", "=", "configoption4")->first()->value;
    $allowipconflict = (int) $model->productAddon->moduleConfiguration()->where("setting_name", "=", "configoption5")->first()->value;
    $allowdirectoryconflict = (int) $model->productAddon->moduleConfiguration()->where("setting_name", "=", "configoption6")->first()->value;
    $licensing_secretkey = $model->productAddon->moduleConfiguration()->where("setting_name", "=", "configoption8")->first()->value;
    $licensing_freetrial = (int) $model->productAddon->moduleConfiguration()->where("setting_name", "=", "configoption9")->first()->value;
    $pid = $model->addonId;
} else {
