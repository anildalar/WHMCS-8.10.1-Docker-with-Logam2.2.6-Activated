          ...
          action: function(){
            ...
            $.ajax({
             ...
             success:function(response){
               ...
               $("#AdminPanel").removeClass().addClass('successbox').show();
              // serverStatus();
               } else {
...
function (response) {
  ...
  function serverAddonBox() {
    $.post(window.location.href, { subaction:"AjaxServerStatus" },
  function (response) {
  var parsed = JSON.parse(response);
    jQuery('#addonbuy').append($('<option>', {
        value: "",
        text: parsed.selAddon
    }));
    jQuery.each(parsed.Serveraddons, function(index1, value1) {
      var gross = Number(value1.product.price.price.gross).toFixed(3);
      var grosssetup = Number(value1.product.price.price_setup.gross).toFixed(3);
                jQuery('#addonbuy').append($('<option>', {
                    value: value1.product.id,
                    text: value1.product.name + " (" + langVar.Price +": "+ gross +" + "+ langVar.Setup+": "+ grosssetup +")"
                }));
        });
    });
  $.confirm({
    type: "blue",
    columnClass: 'medium',
    title: langVar.Server_addons_list,
    content: ""+
    '<div class="modal-body">'+
    '<p><i class="fa fa-info-circle text-danger" aria-hidden="true"></i> '+langVar.addon_Confirm+' ?</p>'+
    '</div>'+
    '<table class="table table-bordered table-hover">'+
    '<tr>'+
    '<td>'+langVar.selAddonlist+':</td>'+
    '<td>'+
    '<select class="form-control" id="addonbuy">'+
    '</select>'+
    '</td>'+
    '</tr>'+
    '</table>'  ,
    ...
    buttons: {
      confirm: {
        ...
        text: "<i class=\'fa fa-check-circle\'></i> "+langVar.Confirm,
        action: function(){
          var addonorder = this.$content.find('#addonbuy').val();
                    if(!addonorder){
                        $.alert(langVar.selAddon);
                        return false;
                    }
          $("#hetzner-loader").show();
           $.ajax({
            url: window.location.href,
            type: 'post',
            data: { subaction:'AjaxOrderAddon', addonorder:addonorder },
          }).done(function(response){
            var parsed = JSON.parse(response);
            if(parsed.success){
            $("#custom-alert-message").html(parsed.success);
    ...
    confirm: {
      ...
      action: function(){
        ...
        $.ajax({
         ...
         $("#AdminPanel").removeClass('').addClass('alert alert-success').show();
         $('#ShowSnapTables').DataTable().ajax.reload().columns.adjust();
         } else {
         $("#custom-alert-message").html(langVar.errormsg);
         $("#AdminPanel").removeClass('alert alert-success').addClass('alert alert-danger').show();
         }
         $("#hetzner-loader").hide();
         $(window).scrollTop(0);
       });
