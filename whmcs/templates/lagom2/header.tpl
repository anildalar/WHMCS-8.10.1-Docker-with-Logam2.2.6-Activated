...
{else}
    <!DOCTYPE html>
    <html {if (isset($display_mode_switcher) && is_array($display_mode_switcher) && $display_mode_switcher['dark'] == "true") || (isset($RSThemes.addonSettings.display_mode_switcher) && $RSThemes.addonSettings.display_mode_switcher == "1" && isset($RSThemes.addonSettings.display_mode_type) && $RSThemes.addonSettings.display_mode_type == "forced")}class="lagom-dark-mode"{/if} lang="{$activeLocale.languageCode}" {if ($language == 'arabic' || $language == 'hebrew' || $language == 'farsi') && file_exists("templates/{$template}/assets/css/theme-rtl.css")}dir="rtl"{/if}>
    <head>
