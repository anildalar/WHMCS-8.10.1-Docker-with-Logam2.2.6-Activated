...
/* * * * * * * * * * * * * * * * *
 *
 *  Promotion Extension Hook
 *  Author: Kamil Miesiąc
 *
 * * * * * * * * * * * * * * * * */
...
add_hook('ClientAreaPage', -1, function ($vars) {
    if (Capsule::schema()->hasTable("rsextension_promobanners_slides") && Capsule::schema()->hasTable("rsextension_translation_content")) {
        return [
            'promoBannerStatus' => Capsule::table('tblconfiguration')->where('setting', 'lagom_promobanners_exstatus')->select('value')->first()->value,
            'promoSliderExtension' => Banner::getSlides(),
            'promotionClose' => Capsule::table('tblconfiguration')->where('setting', 'lagom_promobanners_banner_close')->select('value')->first()->value,
            'promotionCloseTime' => Capsule::table('tblconfiguration')->where('setting', 'lagom_promobanners_banner_close_time')->select('value')->first()->value,
        ];
    }
});
...
add_hook('ClientAreaHeadOutput', 1, function($vars) {
    if (Capsule::schema()->hasTable("rsextension_promobanners_slides") && Capsule::schema()->hasTable("rsextension_translation_content")) {
        ...
        }elseif ($vars['templatefile'] === 'clientareahome' || $vars['templatefile'] === 'viewcart') {
