                                ...
                                <div class="product-recommendation-footer">
                                    <div class="product-recommendation-price">
                                        ...
                                        {if !$recommendation->isFree() && $price}
                                            <h4 class="breakdown-price co jest kurwa">
                                                {if $price->isOneTime()}
                                                    {if $displayPriceSuffix === "on"}{$price->price()->toFull()}{else}{$price->price()->toFull()|replace:$WHMCSCurrency.suffix:""}{/if}
                                                {elseif $price->isRecurring()}
                                                    {if $displayPriceSuffix === "on"}{$price->breakdownPrice()}{else}{$price->breakdownPrice()|replace:$WHMCSCurrency.suffix:""}{/if}
                                                {/if}
                                            </h4>
                                        {/if}
                                        {if !$recommendation->isFree() && !is_null($price->setup()) && $price->setup()->toNumeric() > 0}
                                            <span class="setup-fee"><small>{if $displayPriceSuffix === "on"}{{$price->setup()->toFull()}}{else}{$price->setup()->toFull()|replace:$WHMCSCurrency.suffix:""}{/if}&nbsp;{lang key="ordersetupfee"}</small></span>
                                        {/if}
                                    </div>
