...
{else}
    <div class="section" id="paymentGatewaysDetails">
        ...
        <div class="section-body">
            ...
            <div class="credit-balance"><p class="credit-balance-title">{$LANG.availcreditbal}:</p><span>{if $displayPriceSuffix == "off"}{$creditBalance->toPrefixed()}{else}{$creditBalance}{/if}</span></div>
                            ...
                            <div class="radio">
                                <label>
                                    ...
                                    <span id="spanFullCredit" {if !($creditBalance->toNumeric() >= $total->toNumeric())} class="hidden"{/if}>
                                        {if $displayPriceSuffix == "off"}
                                            {lang key='cart.applyCreditAmountNoFurtherPayment' amount=$total->toPrefixed()}
                                        {else}
                                ...
                                <div class="panel panel-check {if $selectedgateway eq $gateway.sysname}checked{/if} {$gateway.sysname|lower|replace:" ":"-"}" data-virtual-input>
                                    <div class="panel-heading check">
                                        <label>
                                            ...
                                            <span class="check-icon" data-gateway-icon-name="">
                                            {if !empty($gatewayIcons)}
                                                {foreach $gatewayIcons as $icon}
                                                    {if $icon.name == $gateway.sysname}
                                                        <img src="modules/addons/LagomOrderForm/templates/client/default/assets/img/gateways/{$icon.icon_name}">
                                                    {/if}
                                                {/foreach} 
                                            {/if}  
                                            </span>
